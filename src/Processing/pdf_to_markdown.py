...
class PDFParsingModule:
    ...
    def __init__(self, config):
        ...
        self.logger = LogfireLogger(config=config, service_name=__name__) # Initialize logger with config and module name
        self.log_level = self.config.get('log_level', "INFO") # Default log level if not in config
...
class MetadataExtractionModule:
    ...
    def __init__(self, config):
        ...
        self.logger = LogfireLogger(config=config, service_name=__name__) # Initialize logger
...
class TextExtractionModule:
    ...
    def __init__(self, config):
        ...
        self.logger = LogfireLogger(config=config, service_name=__name__) # Initialize logger
        self.text_blocks_strategy = self.config.get('text_blocks_strategy', 'blocks_sorted') # Default strategy, configurable via YAML
...
class TableExtractionModule:
    ...
    def extract_tables_from_page(self, page):
        ...
        """
        table_config = self.config # Get table config from module config
        tables = page.find_tables(**table_config) # Find tables using config from YAML
...
class ImageExtractionModule:
    ...
    def __init__(self, config):
        ...
        self.logger = LogfireLogger(config=config, service_name=__name__) # Initialize logger
        self.image_format = self.config.get('image_format', 'png') # Default image format, configurable via YAML
        self.image_quality = self.config.get('image_quality', 90) # Default image quality, configurable via YAML
        self.output_subdirectory = self.config.get('output_subdirectory', 'assets/images') # Default subdirectory, configurable via YAML
    ...
    def extract_images_from_page(self, page, output_dir):
        ...
        image_output_path = os.path.join(output_dir, self.output_subdirectory) # Construct image output path
        ...
        for img_index, img in enumerate(image_list, start=1): # Iterate through images
            ...
            image_bytes = base_image["image"] # Get raw image bytes
            image_ext = base_image["ext"] # Get image extension (e.g., 'png', 'jpeg')
            image_filename = f"page_{page.number + 1}_image_{img_index}.{self.image_format}" # Construct image filename
            image_path = os.path.join(image_output_path, image_filename) # Construct full image path

            pix = pymupdf.Pixmap(doc, xref) # Create Pixmap object for image
            if pix.n - pix.alpha > 3: # Check for CMYK images and convert to RGB if necessary
                pix = pymupdf.Pixmap(pymupdf.csRGB, pix) # Convert CMYK to RGB

            pix.save(image_path, format=self.image_format, quality=self.image_quality) # Save Pixmap to file with format and quality from config
            ...
            pix = None # Release Pixmap object
...
class MarkdownOutputModule:
    ...
    def __init__(self, config):
        ...
        self.logger = LogfireLogger(config=config, service_name=__name__) # Initialize logger
        self.markdown_filename_suffix = self.config.get('markdown_filename_suffix', '_parsed') # Default suffix, configurable via YAML
        self.asset_subdirectory = self.config.get('asset_subdirectory', 'assets') # Default asset subdirectory, configurable via YAML
    ...
    def create_markdown_image_link(image_filename, asset_subdirectory):
        ...
        """
        return f"![Page Image](<{asset_subdirectory}/{image_filename}>)" # Return Markdown image link string with alt text
...
class MathNotationModule:
    ...
    def __init__(self, config):
        ...
        self.logger = LogfireLogger(config=config, service_name=__name__) # Initialize logger
        self.latex_display_delimiter = self.config.get('latex_display_delimiter', '$$') # Default display delimiter, configurable via YAML
        self.latex_inline_delimiter = self.config.get('latex_inline_delimiter', '$') # Default inline delimiter, configurable via YAML
    ...
    def format_latex_in_markdown(self, text):
        ...
        text = re.sub(r'(\$\$.*?\$\$)', r'\1', text, flags=re.DOTALL) # Wrap display equations in delimiters
        text = re.sub(r'(\$.*?\$)', r'\1', text) # Wrap inline equations in delimiters
...
class CodeSnippetModule:
    ...
    def __init__(self, config):
        ...
        self.logger = LogfireLogger(config=config, service_name=__name__) # Initialize logger
        self.code_fence_style = self.config.get('code_fence_style', '```') # Default code fence style, configurable via YAML
    ...
    def format_code_in_markdown(self, text):
        ...
        for line in lines: # Iterate through lines
            if line.strip().startswith("```") or line.strip().startswith("~~~"): # Check for existing code fences
                ...
                formatted_lines.append(f"{self.code_fence_style}python") # Start code fence with python language
                formatted_lines.append(line) # Append code line
                formatted_lines.append(self.code_fence_style) # End code fence
            elif len(line) > 40 and  " " not in line and not in_code_block: # Heuristic: Long lines with no spaces (potential code)
                formatted_lines.append(f"{self.code_fence_style}") # Start code fence
                formatted_lines.append(line) # Append code line
                formatted_lines.append(self.code_fence_style) # End code fence
